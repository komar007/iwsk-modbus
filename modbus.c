#include "modbus.h"
#include "tty.h"

#include <stdbool.h>
#include <stdlib.h>
#include <string.h>
#include <time.h>
#include <unistd.h>
#include <sys/time.h>

void modbus_idle(struct config *cfg)
{
	/* time in microseconds of t characters */
	usleep(cfg->byte_interval);
}

void modbus_make_frame(struct frame *frame, uint8_t addr, uint8_t func,
		size_t data_len, const uint8_t *data)
{
	frame->len = 4 + data_len;
	frame->data = malloc(frame->len);
	frame->data[0] = addr;
	frame->data[1] = func;
	memcpy(frame->data + 2, data, data_len);
	*(uint16_t*)(frame->data + frame->len - 2) = modbus_crc(frame->data, data_len + 2);
}

/* Table of CRC values for high–order byte */
static unsigned char crc_hi_byte[] = {
	
};
/* Table of CRC values for low–order byte */
static char crc_low_byte[] = {
	
};

uint16_t modbus_crc(const uint8_t *data, size_t len)
{
        uint8_t crc_hi = 0xff;
        uint8_t crc_low = 0xff;
        unsigned idx;
        while (len--) {
                 idx = crc_low ^ *data++;
                 crc_low = crc_hi ^ crc_hi_byte[idx];
                 crc_hi = crc_low_byte[idx];
        }
        return (crc_hi << 8 | crc_low);
}

int modbus_send_frame(struct config *cfg, struct frame *frame)
{
	unsigned to_write = frame->len;
	tty_write(cfg->fd, frame->data, frame->len);
	modbus_idle(cfg);
}

long long int get_time()
{
	struct timeval tv;
	gettimeofday(&tv, NULL);
	return (long long int)tv.tv_sec * 1000000 + tv.tv_usec;
}

int modbus_recv_frame(struct config *cfg, struct frame *frame, int timeout)
{
	uint8_t buf[256];
	long long int t = get_time();
	ssize_t nread;
	while (true) {
		nread = read(cfg->fd, buf, 1);
		if (nread == 1)
			break;
		if (get_time() - t > timeout)
			return -1;
	}
	t = get_time();
	uint8_t *ptr = buf + 1;
	while (true) {
		nread = read(cfg->fd, ptr, 1);
		if (nread == 1) {
			++ptr;
			t = get_time();
		} else if (get_time() - t > cfg->byte_interval_limit) {
			break;
		}
	}
	frame->len = ptr - buf;
	frame->data = malloc(frame->len);
	memcpy(frame->data, buf, frame->len);
	return frame->len;
}

int modbus_open(struct config *cfg, const char *dev, int baud,
		int byte_interval, int byte_interval_limit)
{
	cfg->baud = baud;
	cfg->byte_interval = byte_interval;
	cfg->byte_interval_limit = byte_interval_limit;
	cfg->fd = tty_open(dev);
	if (cfg->fd < 0)
		return -1;
	uint8_t buf[1];
	while (read(cfg->fd, buf, 1) > 0)
		;
	return 0;
}

void modbus_print_frame(struct frame *frame)
{
	printf("frame hex dump:\n");
	for (unsigned i = 0; i < frame->len; ++i)
		printf("%.2x", frame->data[i]);
	printf("\nframe info:\n");
	printf("addr = %.2x, function = %.2x, payload len = %zi\n",
			modbus_get_frame_addr(frame),
			modbus_get_frame_func(frame),
			modbus_get_frame_payload_len(frame));
}

bool modbus_frame_correct(struct frame *frame)
{
	uint16_t crc = *(uint16_t*)&frame->data[frame->len - 2];
	uint16_t computed_crc = modbus_crc(frame->data, frame->len - 2);
	return computed_crc == crc;
}

uint8_t modbus_get_frame_addr(struct frame *frame)
{
	return frame->data[0];
}

uint8_t modbus_get_frame_func(struct frame *frame)
{
	return frame->data[1];
}

uint8_t *modbus_get_frame_payload(struct frame *frame)
{
	return &frame->data[2];
}

size_t modbus_get_frame_payload_len(struct frame *frame)
{
	return frame->len - 4;
}

void modbus_free_frame(struct frame *frame)
{
	free(frame->data);
	frame->len = 0;
}
